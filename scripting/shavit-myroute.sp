...
void GetMyRoute(int client)
{
    ...
    if(gRT_RouteType[client] == RouteType_ServerRecord || (!FileExists(gS_ReplayPath[client]) && gRT_RouteType[client] == RouteType_Auto))
    {
    ...
    PrintDebug("Found [%N]'s route path. Type: [%s] | Path [%s]", client, type, gS_ReplayPath[client]);
}
...
public void Shavit_OnWorldRecord(int client, int style, float time, int jumps, int strafes, float sync, int track, float oldwr, float oldtime, float perfs, float avgvel, float maxvel, int timestamp)
{
    for(int i = 1; i <= MaxClients; i++)
    {
        ...
        if(gA_FrameCache[i].iSteamID != SteamIDToAccountID(steamID) || i == client)
        {
            LoadMyRoute(i);
        }
    }
...
public int MyRoute_MenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
    switch(action)
    {
        case MenuAction_Select:
        {
            ...
            {
                if(gRT_RouteType[param1] + view_as<RouteType>(1) >= RouteType_Size)
                {
